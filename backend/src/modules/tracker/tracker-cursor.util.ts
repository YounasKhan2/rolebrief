...
export interface TrackerCursorPayload {
  v: 1;
  userId: string;
  stageFilter: string;
  updatedAt: string;
...
export function encodeTrackerCursor(payload: TrackerCursorPayload, secret: string): string {
...
export function decodeTrackerCursor(
  cursor: string,
  secret: string,
  ...
  const normExpectedStage = expectedStageFilter || "";
  const normParsedStage = parsed.stageFilter || "";
  if (normParsedStage !== normExpectedStage) {
