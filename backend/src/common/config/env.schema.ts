...
export const envSchema = z.object({
  ...
  RESUME_PROCESSING_MAX_ATTEMPTS: z.coerce.number().int().positive().default(3),
  RESUME_PROCESSING_LEASE_SECONDS: z.coerce.number().int().positive().default(120),
  RESUME_PROCESSING_TIMEOUT_MS: z.coerce.number().int().positive().default(20000),
  RESUME_PDF_MAX_PAGES: z.coerce.number().int().positive().default(20),
  RESUME_DOCX_MAX_ENTRIES: z.coerce.number().int().positive().default(300),
  RESUME_DOCX_MAX_UNCOMPRESSED_BYTES: z.coerce.number().int().positive().default(20 * 1024 * 1024),
  RESUME_DOCX_MAX_ENTRY_BYTES: z.coerce.number().int().positive().default(5 * 1024 * 1024),
  RESUME_DOCX_MAX_COMPRESSION_RATIO: z.coerce.number().int().positive().default(100),
  CLAMAV_HOST: z.string().min(1).default("localhost"),
  CLAMAV_PORT: z.coerce.number().int().positive().default(3310),
  CLAMAV_SCAN_TIMEOUT_MS: z.coerce.number().int().positive().default(15000),
  EXTRACTION_SERVICE_URL: z.string().url().default("http://localhost:8080"),
  EXTRACTION_INTERNAL_TOKEN: z.string().min(16).default("replace-with-extraction-internal-token"),
  EXTRACTION_REQUEST_TIMEOUT_MS: z.coerce.number().int().positive().default(30000),
  EXTRACTION_RESPONSE_MAX_BYTES: z.coerce.number().int().positive().default(5 * 1024 * 1024),
  RESUME_EXTRACTION_MAX_BLOCKS: z.coerce.number().int().positive().default(1500),
  RESUME_EXTRACTION_MAX_TEXT_BYTES: z.coerce.number().int().positive().default(2 * 1024 * 1024),
  RESUME_EXTRACTION_MAX_BLOCK_TEXT_BYTES: z.coerce.number().int().positive().default(8000),
  // Minimum character count from pdfminer.six before OCR is attempted (ocrPolicy=auto).
  RESUME_EXTRACTION_OCR_MIN_CHARS: z.coerce.number().int().nonnegative().default(100),
  RESUME_EXTRACTION_MAX_RECONCILE: z.coerce.number().int().positive().default(25),
