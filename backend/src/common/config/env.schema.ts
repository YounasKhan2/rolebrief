...
export const envSchema = z.object({
  ...
  DOCLING_SERVICE_URL: z.string().url().default("http://localhost:8080"),
  DOCLING_INTERNAL_TOKEN: z.string().min(16).default("replace-with-docling-internal-token"),
  DOCLING_REQUEST_TIMEOUT_MS: z.coerce.number().int().positive().default(30000),
  DOCLING_RESPONSE_MAX_BYTES: z.coerce.number().int().positive().default(5 * 1024 * 1024),
  ...
  RESUME_EXTRACTION_OCR_MIN_BLOCKS: z.coerce.number().int().nonnegative().default(3),
