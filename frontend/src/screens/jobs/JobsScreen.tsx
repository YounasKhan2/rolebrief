...
export function Component() {
  const candidate = useLocation().pathname.startsWith("/app/");
  ...
  const sort = (params.get("sort") ?? (urlQ ? "relevance" : "newest")) as
    | "best"
    | "newest"
    | "relevance";
  const density = (params.get("density") ?? "comfortable") as
    | "comfortable"
    | "compact";
  ...
  const disc = useMemo(
    () => new Set(discParam.split(",").filter(Boolean)),
    [discParam],
  );
  const remote = useMemo(
    () => new Set(remoteParam.split(",").filter(Boolean)),
    [remoteParam],
  );
  const senior = useMemo(
    () => new Set(seniorParam.split(",").filter(Boolean)),
    [seniorParam],
  );
  const elig = useMemo(
    () => new Set(eligParam.split(",").filter(Boolean)),
    [eligParam],
  );
  ...
  const { summaries: eligibilitySummaries, loading: eligibilityLoading } =
    useBatchEligibility(visibleSlugs, isAuthenticated && !isAdmin);
  const {
    summaries: matchSummaries,
    loading: matchLoading,
    error: matchError,
    retry: retryMatch,
  } = useBatchMatchBriefs(visibleSlugs, auth.user, isAuthenticated && !isAdmin);
  ...
  const filterRail = (
    <div
      className={classNames("space-y-6", candidate && "candidate-job-filters")}
    >
      <FilterGroup
        title="Discipline"
        onClear={disc.size ? () => update({ disc: null }) : undefined}
      >
        {disciplines.map((d) => (
          <FilterChip
            key={d}
            active={disc.has(d)}
            onClick={() => toggleMulti("disc", d)}
          >
            {d}
          </FilterChip>
        ))}
      ...
      <FilterGroup
        ...
        onClear={remote.size ? () => update({ remote: null }) : undefined}
      >
        {remoteFilters.map((r) => (
          <FilterChip
            key={r.key}
            active={remote.has(r.key)}
            onClick={() => toggleMulti("remote", r.key)}
          >
            {r.label}
          </FilterChip>
        ))}
      ...
      <FilterGroup
        title="Experience"
        onClear={senior.size ? () => update({ senior: null }) : undefined}
      >
        {seniorities.map((s) => (
          <FilterChip
            key={s}
            active={senior.has(s)}
            onClick={() => toggleMulti("senior", s)}
          >
            {s}
          </FilterChip>
        ))}
      ...
      {!candidate && (
        <FilterGroup
          title="Eligibility Shield"
          onClear={elig.size ? () => update({ elig: null }) : undefined}
        >
          {eligibilityFilters.map((e) => (
            <FilterChip
              key={e.key}
              active={elig.has(e.key)}
              onClick={() => toggleMulti("elig", e.key)}
            >
              {e.label}
            </FilterChip>
          ))}
      ...
      <FilterGroup title="Salary">
        <FilterChip
          active={salaryOnly}
          onClick={() => update({ salary: salaryOnly ? null : "1" })}
        >
  ...
  return (
    <PageContainer className={candidate ? "candidate-jobs" : undefined}>
      {candidate && (
        <header className="radar-page-header jobs-page-header">
          <div>
            ...
            <p>
              Search current roles and compare the evidence before you apply.
            </p>
          </div>
          <Link to="/app/radar" className="jobs-radar-link">
            Open your Radar <ArrowUpDown size={13} />
          </Link>
        </header>
      )}
      {/* Sticky search + quick controls */}
      <div className="jobs-search-toolbar sticky top-16 z-30 -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8 py-3 bg-white/90 backdrop-blur border-b border-line">
        <div className="flex items-center gap-3">
            ...
            <Input
              ...
              placeholder={
                candidate
                  ...
                  : "Search title or skill — e.g. React, backend, Karachi"
              }
              type="search"
          ...
          <Button
            variant="secondary"
            className="jobs-mobile-filter md:hidden"
          ...
          <div className="jobs-search-controls hidden md:flex items-center gap-2">
            <SegmentedControl
          ...
          <div className="flex flex-wrap items-center gap-2 mt-3">
            {[...disc].map((d) => (
              <ActiveChip
                key={d}
                label={d}
                onRemove={() => toggleMulti("disc", d)}
              />
            ))}
            {[...remote].map((r) => (
              <ActiveChip
                key={r}
                label={remoteFilters.find((x) => x.key === r)?.label ?? r}
                onRemove={() => toggleMulti("remote", r)}
              />
            ))}
            {[...senior].map((s) => (
              <ActiveChip
            ...
            {!candidate &&
              [...elig].map((e) => (
                <ActiveChip
                  key={e}
                  label={
                    eligibilityFilters.find((x) => x.key === e)?.label ?? e
                  }
                  onRemove={() => toggleMulti("elig", e)}
                />
              ))}
            {salaryOnly && (
              <ActiveChip
                label="Employer-provided salary"
                onRemove={() => update({ salary: null })}
              />
            )}
            <button
              onClick={clearAll}
              className="text-[13px] text-slate hover:text-red ml-1"
            >
              Clear all
            </button>
          </div>
      ...
      <div className="jobs-layout mt-6 grid min-w-0 lg:grid-cols-[260px_1fr] gap-8 items-start">
        {/* Left filter rail (desktop) */}
        <aside
          className="jobs-filter-rail hidden lg:block lg:sticky lg:top-36"
        ...
        >
          {candidate && (
            <div className="jobs-filter-heading">
              ...
              <button onClick={clearAll} disabled={!activeCount && !urlQ}>
                Reset
              </button>
            </div>
          )}
          {filterRail}
          {candidate && (
            <p className="jobs-filter-note">
              Match Brief explains alignment. Eligibility checks restrictions.
              Both are shown separately on each role.
            </p>
          )}
        </aside>

        <div className="jobs-results min-w-0">
          <div className="jobs-results-heading flex items-center justify-between mb-4">
              ...
              {loading ? (
                ...
                <>
                  ...
                  {urlQ && (
                    <>
                      {" "}
                      for "<span className="text-ink">{urlQ}</span>"
                    </>
                  )}
                </>
              )}
              ...
              <Button
                ...
                onClick={() =>
                  toast({
                    kind: "info",
                    message:
                      "Saved searches are unavailable until the next phase.",
                  })
                }
              >
                Save search
            ...
            {candidate && (
              <Link to="/app/alerts" className="jobs-alert-link">
                Manage alerts
              </Link>
            )}
          </div>
          ...
          {candidate && matchError && (
            <div className="radar-notice" role="status">
              <div>
                <strong>Match Brief is temporarily unavailable.</strong>
                <p>
                  You can still review roles and their eligibility evidence.
                </p>
              </div>
              <Button variant="secondary" size="sm" onClick={retryMatch}>
                Retry Match Brief
              </Button>
            </div>
          )}
          ...
          {loading ? (
            ...
            <EmptyState
              ...
              className={candidate ? "jobs-result-state" : undefined}
              headingLevel={candidate ? "h2" : "h3"}
              body={`${error.message}${error.status === 429 && error.retryAfterSeconds ? ` Try again in ${error.retryAfterSeconds} seconds.` : ""}`}
              action={
            ...
            <EmptyState
              ...
              headingLevel={candidate ? "h2" : "h3"}
              body={
                totalCount === 0 && !urlQ && activeCount === 0
                  ? "No stored Himalayas jobs are available yet."
                  : "Try clearing or broadening your search terms and filters."
              }
              action={
                <Button
                  variant="secondary"
                  ...
                  candidate ? (
                    <CandidateJobCard
                      ...
                      job={{
                        ...
                        match: {
                          ...j.match,
                          summary: matchSummaries.get(j.slug),
                        },
      ...
      {/* Mobile filter sheet */}
      <Sheet
        className={candidate ? "candidate-jobs-sheet" : undefined}
        manageFocus={candidate}
        open={filtersOpen}
        onClose={() => setFiltersOpen(false)}
        title="Filters"
        footer={
          <>
            <Button variant="secondary" className="grow" onClick={clearAll}>
              Clear all
            </Button>
    ...
    >
      {Array.from({ length: count }).map((_, index) => (
        ...
        >
          ...
          {density === "comfortable" && (
            <div className="mt-4 h-14 rounded-[10px] bg-soft animate-pulse" />
          )}
        </div>
      ))}
    </div>
    ...
    <div>
      <div className="flex items-center justify-between mb-2.5">
        ...
        {onClear && (
          <button
            onClick={onClear}
            className="text-[12px] text-slate hover:text-red"
          >
            Clear
          </button>
        )}
